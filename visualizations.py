...
import tracker
from util import tab_manager, create_download_buttons_plotly, create_download_buttons_matplotlib, display_chart_with_expander
...
def efficient_value_counts(df, col, max_categories=1000):
    """Safe and consistent value counts"""
    ...
    try:
        ...
        with tab1:
            ...
            try:
                ...
                with st.expander("📊 Missing Data Pattern Matrix", expanded=True):
                    st.pyplot(fig)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_matplotlib(fig, "missing_data_matrix", "vis_matrix")
        ...
        with tab2:
            ...
            try:
                ...
                ax.set_title("Missing Data Correlation", pad=20,
                            fontdict={'fontsize': 16, 'color': PROFESSIONAL_PALETTE['primary']})
                with st.expander("📊 Missing Data Correlation Heatmap", expanded=True):
                    st.pyplot(fig)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_matplotlib(fig, "missing_data_heatmap", "vis_heatmap")
        ...
        with tab3:
            ...
            try:
                ...
                with st.expander("📊 Missing Values Count by Column", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "missing_values_bar", "vis_missing_bar")
    ...
    try:
        ...
        with tab1:
            ...
            try:
                ...
                with st.expander(f"📊 Distribution of {selected_col}", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, f"distribution_{selected_col}", "vis_dist_hist")
        ...
        with tab2:
            ...
            try:
                ...
                with st.expander(f"📊 Q-Q Plot for {selected_col}", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, f"qq_plot_{selected_col}", "vis_qq_plot")
        ...
        with tab3:
            ...
            try:
                ...
                with st.expander(f"📊 Box & Violin Plot for {selected_col}", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, f"box_violin_{selected_col}", "vis_box_violin")
    ...
    try:
        ...
        with tab1:
            ...
            try:
                ...
                with st.expander("📊 Correlation Heatmap", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "correlation_heatmap", "vis_corr_heat")
        ...
        with tab2:
            ...
            try:
                ...
                with st.expander("📊 Scatter Matrix", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "scatter_matrix", "vis_scatter_matrix")
        ...
        with tab3:
            ...
            try:
                ...
                with st.expander("📊 Interactive Pair Plot", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "pair_plot", "vis_pair_plot")
    ...
    try:
        ...
        with tab1:
            ...
            try:
                ...
                with st.expander("📊 Categorical Distribution - Bar Chart", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "categorical_bar_chart", "cat_bar_chart")
                tracker.log_operation("Viewed Bar Chart")
            except Exception as e:
        ...
        with tab2:
            ...
            try:
                ...
                with st.expander("📊 Categorical Distribution - Pie Chart", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "categorical_pie_chart", "cat_pie_chart")
        ...
        with tab3:
            ...
            try:
                ...
                with st.expander("📊 Categorical Distribution - Treemap", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "categorical_treemap", "cat_treemap")
    ...
    try:
        ...
        with tab1:
            ...
            if len(numeric_cols) <= 2:
                ...
                if len(selected_cols) >= 2:
                    ...
                    with st.expander("📊 Parallel Coordinates Plot", expanded=True):
                        st.plotly_chart(fig, use_container_width=True)
                        st.markdown("**Download Chart:**")
                        create_download_buttons_plotly(fig, "parallel_coordinates", "adv_parallel")
        ...
        with tab2:
            ...
            if len(numeric_cols) < 3:
                ...
                with st.expander("📊 Radar Chart of Feature Values", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "radar_chart", "adv_radar")
            ...
            if len(categorical_cols) < 2:
                ...
                if len(selected_cats) >= 2:
                    try:
                        ...
                        with st.expander("📊 Hierarchical Sunburst Chart", expanded=True):
                            st.plotly_chart(fig, use_container_width=True)
                            st.markdown("**Download Chart:**")
                            create_download_buttons_plotly(fig, "sunburst_chart", "adv_sunburst")
                        tracker.log_operation("Viewed Sunburst Chart")
                    except Exception as e:
                        msg = str(e)
                        if "Non-leaves rows are not permitted in the dataframe" in msg:
                            st.warning("Some rows in your data are not leaves in the hierarchy. Attempting to filter only leaf rows for sunburst chart...")
                            # Try to filter only leaf rows
                            try:
                                # A row is a leaf if it is unique in the selected path
                                leaf_mask = ~df_vis.duplicated(subset=selected_cats, keep=False)
                                leaf_df = df_vis[leaf_mask]
                                if leaf_df.empty:
                                    st.error("No leaf rows found in the selected hierarchy. Please adjust your selection.")
                                else:
                                    fig = px.sunburst(
                                        leaf_df,
                                        path=selected_cats,
                                        title="Hierarchical Sunburst Chart (Leaf Rows Only)",
                                        color_discrete_sequence=PROFESSIONAL_PALETTE['diverging']
                                    )
                                    fig.update_layout(
                                        plot_bgcolor=PROFESSIONAL_PALETTE['background'],
                                    ...
                                    with st.expander("📊 Hierarchical Sunburst Chart (Leaf Rows Only)", expanded=True):
                                        st.plotly_chart(fig, use_container_width=True)
                                        st.markdown("**Download Chart:**")
                                        create_download_buttons_plotly(fig, "sunburst_chart_leaf", "adv_sunburst_leaf")
                                    tracker.log_operation("Viewed Sunburst Chart (Leaf Rows Only)")
                            except Exception as e2:
                                st.error(f"Error after filtering for leaf rows: {e2}")
                        else:
                            st.error(f"Error in Sunburst Chart: {e}")
        ...
        with tab4:
            ...
            if len(numeric_cols) < 2:
                ...
                with st.expander("📊 2D Bubble Chart with RGB Coloring", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "bubble_2d_chart", "adv_bubble_2d")
        ...
        with tab5:
            ...
            if df_vis.empty:
                ...
                with st.expander("📊 3D Bubble Chart with RGB Coloring", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "bubble_3d_chart", "adv_bubble_3d")
