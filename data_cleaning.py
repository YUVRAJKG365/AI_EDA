...
def clean_data(df, cleaning_method, fill_value, drop_axis, remove_duplicates=False,
    ...
    if remove_outliers_col and remove_outliers_col in df.columns and pd.api.types.is_numeric_dtype(
            df[remove_outliers_col]):
...
def render_data_cleaning_section():
    ...
    if (
        ...
        st.warning(
            "⚠️ Please upload a CSV or Excel file to use Data Cleaning features. Data cleaning is only available for structured tabular data (CSV/Excel).")
