...
def home_ui():
    # Initialize tracker in session state if not already present
    if 'tracker' not in st.session_state:
        st.session_state.tracker = AppTracker()

    # Log the section visit
    st.session_state.tracker.log_section("Home")
    ...
    with st.expander("📁 Upload Data", expanded=True):
        # Log the tab opening
        st.session_state.tracker.log_tab("Upload Data")

        uploader_key = f"file_uploader_{st.session_state.file_uploader_key}"
        uploaded_file = st.file_uploader(
            "Choose Document File or Image File",
            type=["csv", "xlsx", "xls", "txt", "pdf", "png", "jpg", "jpeg", "bmp", "tiff", "gif", "webp", "docx",
                  "pptx"],
            key=uploader_key,
        ...
        if uploaded_file is not None:
            ...
            st.session_state.tracker.log_operation(f"Uploaded file: {uploaded_file.name}")
    ...
    if st.session_state.get('file_processed', False):
        ...
        if st.button("🗑️ Remove Uploaded File", key="remove_file_btn"):
            # Log file removal operation
            st.session_state.tracker.log_operation("Removed uploaded file")
    ...
    if st.session_state.get('file_processed', False):
        ...
        if 'df' in st.session_state and st.session_state.df is not None:
            ...
            # Log that we're viewing tabular data information
            st.session_state.tracker.log_tab("Tabular Data Information")

            with st.expander("📐 Dataset Shape & Columns", expanded=True):
                st.session_state.tracker.log_subtab("Dataset Shape & Columns")
            ...
            with st.expander("🔤 Data Types", expanded=True):
                st.session_state.tracker.log_subtab("Data Types")
            ...
            with st.expander("❓ Missing Values", expanded=True):
                st.session_state.tracker.log_subtab("Missing Values")
            ...
            with st.expander("📈 Data Summary", expanded=True):
                st.session_state.tracker.log_subtab("Data Summary")
                plot_data_summary(df)

            with st.expander("👀 Data Preview", expanded=True):
                st.session_state.tracker.log_subtab("Data Preview")
                st.dataframe(df)

            with st.expander("📊 Quick Statistics", expanded=True):
                st.session_state.tracker.log_subtab("Quick Statistics")
                st.write(df.describe(include='all'))

            # Statistical Analysis Section
            with st.expander("📊 Statistical Analysis", expanded=False):
                st.session_state.tracker.log_subtab("Statistical Analysis")
                ...
                if len(numeric_cols) + len(categorical_cols) < 2:
                    ...
                    if st.button("📊 Run Statistical Test", key="stat_test_btn"):
                        st.session_state.tracker.log_operation(
                            f"Ran statistical test: {test_type} on columns {selected_col1} and {selected_col2}"
                        )
                        with st.spinner("Performing test..."):
                            try:
                                # Convert datetime columns to numeric if needed
                                ...
                                for col in [selected_col1, selected_col2]:
                                    if np.issubdtype(temp_df[col].dtype, np.datetime64):
                                        temp_df[col] = temp_df[col].astype(
        ...
        # Text data (txt, docx, pptx)
            ...
            # Log that we're viewing text data information
            st.session_state.tracker.log_tab("Text Data Information")

            with st.expander("📝 Text File Information", expanded=True):
                st.session_state.tracker.log_subtab("Text File Information")
        ...
        # PDF data
            ...
            # Log that we're viewing PDF data information
            st.session_state.tracker.log_tab("PDF Data Information")
            ...
            if pdf_data['text']:
                with st.expander("📝 Extracted Text", expanded=True):
                    st.session_state.tracker.log_subtab("Extracted Text")
            ...
            if pdf_data['images']:
                with st.expander(f"🖼️ Extracted Images ({len(pdf_data['images'])})", expanded=True):
                    st.session_state.tracker.log_subtab("Extracted Images")
            ...
            try:
                ...
                with fitz.open(tmp_path) as doc:
                    ...
                    with st.expander("📋 PDF Metadata", expanded=False):
                        st.session_state.tracker.log_subtab("PDF Metadata")
        ...
        # Image data
            ...
            # Log that we're viewing image data information
            st.session_state.tracker.log_tab("Image Data Information")

            image = st.session_state.image_data
            with st.expander("🖼️ Image Information", expanded=True):
                st.session_state.tracker.log_subtab("Image Information")
                # Display image
                buffered = BytesIO()
