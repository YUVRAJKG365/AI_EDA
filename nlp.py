...
from util import display_extracted_links, display_chart_with_expander, create_download_buttons_plotly
from util import display_extracted_links
...
def perform_advanced_sentiment_analysis(df, text_col, method='distilbert'):
    ...
    try:
        ...
        if text_col not in df.columns:
            st.error(f"❌ Column '{text_col}' not found in dataset!")
            return pd.DataFrame()
        
        # Convert text column to string and handle various data types
        df[text_col] = df[text_col].astype(str).fillna("").str.strip()
        
        # Filter out empty strings
        non_empty_mask = df[text_col].str.len() > 0
        if not non_empty_mask.any():
            st.warning("⚠️ No valid text data found in the selected column!")
            return pd.DataFrame()
        ...
        if len(df) < original_len:
            st.info(f"✓ Filtered out {original_len - len(df)} empty/null entries")
        
    except Exception as e:
        st.error(f"❌ Error validating input data: {str(e)}")
        return pd.DataFrame()
    ...
    def preprocess_text(text):
        try:
            ...
            st.warning(f"⚠️ Error preprocessing text: {str(e)}")
            return str(text)
        ...
        with st.spinner('⚙️ Loading model...'):
            if method == 'distilbert':
                ...
                try:
                    ...
                    st.warning(f"⚠️ Ensemble model not available ({str(e)}), falling back to RoBERTa...")
                    sentiment_pipeline = pipeline(
                        "sentiment-analysis",
                        model="siebert/sentiment-roberta-large-english"
                    )
    ...
    def analyze_sentiment_batch(texts, pipeline):
        """Safely analyze sentiment with text length handling"""
        try:
            ...
            st.warning(f"⚠️ Batch processing error: {str(e)}")
            return [{'label': 'NEUTRAL', 'score': 0.5}] * len(texts)
    ...
    with st.spinner('🧠 Analyzing sentiment...'):
        for idx, batch in enumerate(text_batches):
            try:
                ...
                st.warning(f"⚠️ Error in batch {idx + 1}: {str(e)}")
    ...
    try:
        ...
        st.error(f"⚠️ Error processing sentiment results: {str(e)}")
        df['sentiment_label'] = ['NEUTRAL'] * len(df)
        df['sentiment_score'] = [0] * len(df)

    # Standardize labels across different models
    df['sentiment'] = df['sentiment_label'].apply(
        lambda x: 'positive' if str(x).upper() in ['POSITIVE', 'LABEL_2'] else
        'negative' if str(x).upper() in ['NEGATIVE', 'LABEL_0'] else 'neutral')
    ...
    with tab1:
        ...
        with st.expander("📊 Sentiment Distribution Chart", expanded=True):
            st.plotly_chart(fig, use_container_width=True)
            st.markdown("**Download Chart:**")
            create_download_buttons_plotly(fig, "sentiment_distribution", "nlp_sentiment_pie")
    ...
    with tab2:
        ...
        try:
            ...
            with st.expander("📊 Sentiment Distribution Bar Chart", expanded=True):
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**Download Chart:**")
                create_download_buttons_plotly(fig, "sentiment_bar_chart", "nlp_sentiment_bar")
        except Exception as e:
            st.error(f"⚠️ Error creating sentiment distribution chart: {str(e)}")

        # Polarity distribution histogram
        try:
            if len(df) > 0 and 'sentiment_score' in df.columns:
                fig = px.histogram(df, x='sentiment_score', nbins=min(30, len(df)),
                ...
                with st.expander("📊 Sentiment Score Distribution", expanded=True):
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("**Download Chart:**")
                    create_download_buttons_plotly(fig, "sentiment_score_histogram", "nlp_sentiment_histogram")
            else:
                st.warning("⚠️ Not enough data for histogram")
        except Exception as e:
            st.error(f"⚠️ Error creating histogram: {str(e)}")
    ...
    with tab3:
        ...
        with col1:
            ...
            try:
                ...
                if positive_text and len(positive_text) > 20:
                    ...
                    st.info("✓ Not enough positive text for word cloud")
            except Exception as e:
                st.warning(f"⚠️ Could not generate positive word cloud: {str(e)}")

        with col2:
            st.markdown("#### Negative Sentiment")
            try:
                negative_text = " ".join(df[df['sentiment'] == 'negative']['cleaned_text'].tolist())
                if negative_text and len(negative_text) > 20:
                    wordcloud = WordCloud(width=800, height=400,
                                          background_color='white',
                    ...
                    st.info("✓ Not enough negative text for word cloud")
            except Exception as e:
                st.warning(f"⚠️ Could not generate negative word cloud: {str(e)}")
    ...
    with tab4:
                ...
                if len(most_positive) > 0:
                    for idx, row in most_positive.iterrows():
                        try:
                            ...
                            st.warning(f"Error displaying feedback: {str(e)}")
                else:
                    st.info("No positive feedback found")
            except Exception as e:
                st.error(f"⚠️ Error displaying positive feedback: {str(e)}")
        ...
        with col2:
            ...
            try:
                ...
                st.error(f"⚠️ Error displaying negative feedback: {str(e)}")
        ...
        try:
            if positive_words:
                ...
                st.info("✓ No positive words found in text")
            ...
            if negative_words:
                ...
                st.info("✓ No negative words found in text")
        except Exception as e:
            st.error(f"⚠️ Error creating word frequency charts: {str(e)}")
    ...
    if total == 0:
        ...
        try:
            mode_result = df["sentiment"].mode()
            if len(mode_result) > 0:
                majority = mode_result[0]
                ...
                avg_score = float(df['sentiment_score'].mean()) if len(df['sentiment_score']) > 0 else 0
                sentiment_strength = ""
                ...
                conclusion = "Unable to determine sentiment from data."
        except Exception as e:
            st.warning(f"⚠️ Error calculating conclusion: {str(e)}")
            conclusion = "Analysis completed with partial results."

    st.success(conclusion)

    # Return results with all available columns
    result_cols = [col for col in [text_col, 'sentiment', 'sentiment_score', 'confidence'] if col in df.columns]
...
def show_text_stats(df, text_col):
    ...
    with tab1:
        ...
        if top_unigrams:  # Only plot if we have results
            ...
            with st.expander("📊 Unigrams Visualization", expanded=True):
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**Download Chart:**")
                create_download_buttons_plotly(fig, "top_10_unigrams", "nlp_unigrams")
        else:
    ...
    with tab2:
        ...
        if top_bigrams:  # Only plot if we have results
            ...
            with st.expander("📊 Bigrams Visualization", expanded=True):
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**Download Chart:**")
                create_download_buttons_plotly(fig, "top_10_bigrams", "nlp_bigrams")
        else:
    ...
    with tab3:
        ...
        if top_trigrams:  # Only plot if we have results
            ...
            with st.expander("📊 Trigrams Visualization", expanded=True):
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("**Download Chart:**")
                create_download_buttons_plotly(fig, "top_10_trigrams", "nlp_trigrams")
        else:
            st.warning("No trigrams found")

    st.divider()

    # Display extracted links from all text
    combined_text = " ".join(df[text_col].astype(str).tolist())
    display_extracted_links(combined_text, section_key="nlp_stats")
...
def render_nlp_section():
    ...
    import pandas as pd
    if df_nlp is not None and isinstance(df_nlp, pd.DataFrame):
