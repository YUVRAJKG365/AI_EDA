...
def time_series_analysis(df, date_col, value_col):
    ...
    try:
        ...
        if not df.empty:
            st.write(
                f"**Data Points:** {len(df):,} | **Time Range:** {df.index.min().date()} to {df.index.max().date()}")
            ...
            with st.spinner("Detecting anomalies..."):
                if anomaly_method == 'Z-Score':
                    ...
                    anomalies = anomaly_df[
                        (anomaly_df[value_col] < lower_bound) | (anomaly_df[value_col] > upper_bound)]
    ...
    if df is not None:
        ...
        if len(datetime_cols) > 0 and len(numeric_cols) > 0:
            ...
            if len(df) > 10000 or len(df.columns) > 50:
                st.warning(
                    f"Large dataset detected ({len(df):,} rows, {len(df.columns)} columns). Some operations may be slower.")
            ...
            st.info(
                "The dataset requires at least one datetime column and one numeric column for time series analysis.")
