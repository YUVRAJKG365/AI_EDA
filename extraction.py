...
from util import display_extracted_links, create_download_buttons_plotly, create_download_buttons_matplotlib
        ...
        with tab_text:
            ...
            if extraction_result['text']:
                ...
                with stat_col4:
                    ...
                    st.metric("Avg Word Length", avg_word_len)
                ...
                with display_col1:
                    view_type = st.radio(
                        "View Format",
                        ["Formatted Text", "Raw Text", "Paragraph View"],
                        key=f"{section}_view_type"
                    )
                
                with display_col2:
                    # Copy to clipboard button
                    if st.button("📋 Copy Text", key=f"{section}_copy_btn"):
                ...
                with display_col3:
                    ...
                    st.download_button(
                        ...
                        data=text_content,
                        file_name=f"extracted_text_{extraction_result['file_name']}.txt",
                        mime="text/plain",
                        key=f"{section}_download_txt"
                    )
                
                st.divider()
                
                # Display text based on selected view type
                if view_type == "Raw Text":
                    st.text_area(
                        "Raw Text Content",
                        value=text_content,
                        height=500,
                    ...
                    # Create a formatted display with markdown
                    formatted_text = text_content.replace('\n', '  \n')
                    st.markdown(formatted_text)
                
                st.divider()
                
                # Display extracted links and emails
                display_extracted_links(text_content, section_key=f"{section}_extracted")
